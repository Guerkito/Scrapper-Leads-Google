...
class Orchestrator:
    ...
    async def buscar_fuente(self, fuente: BaseSource, queries: List[str], punto: str, ciudad_nombre: str, context, limit: int = 20) -> List[Lead]:
        ...
        for q in queries:
            ...
            async with self.semaphore:
                ...
                try:
                    self._log(f"🔍 [{fuente.__class__.__name__}] Buscando: {q} en {punto if not punto.startswith('coord:') else ciudad_nombre}...")

                    def _on_lead_captured(lead_obj):
                        if self.stop_requested: return
                        try:
                            # Asegurar que el lead tenga el nombre de la ciudad, no las coordenadas
                            if lead_obj.ciudad.startswith("coord:"):
                    ...
                    # Asegurar consistencia en los resultados retornados también
                    for l in leads:
                        if l.ciudad.startswith("coord:"):
                            l.ciudad = ciudad_nombre
            ...
            for ciudad in ciudades:
                ...
                for punto in puntos_gps:
                    ...
                    for f in self.fuentes:
                        ...
                        res = await self.buscar_fuente(f, queries, punto, ciudad, context, limit=limit)
